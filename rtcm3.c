...
static const double intervalOfIon[4]={
    0.5, 1, 5, 10
};
...
/* decode type 4011: bds ephemerides -----------------------------------------*/
static int decode_type4011(rtcm_t *rtcm)
{
    ...
    if (i+476<=rtcm->len*8) {
        ...
        eph.f1    =getbits(rtcm->buff,i,11)*P2_50;        i+=11;
        ...
        eph.tgd[0]=getbits(rtcm->buff,i,10)*P2_31;        i+=10;
        eph.tgd[1]=getbits(rtcm->buff,i,10)*P2_31;        i+=10;

    }
    else {
        trace(2,"rtcm3 4011 length error: len=%d\n",rtcm->len);
        return -1;
    }
    /*
    if (prn>=40) {
    ...
    */
    trace(4,"decode_type4011: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        trace(2,"rtcm3 4011 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
    eph.week=adjgpsweek(week);
...
static int decode_ssr1(rtcm_t *rtcm, int sys)
{
    ...
    switch (sys) {
        ...
        case SYS_CMP: np=6; offp=  0; break; /* add BDS */
...
/* decode gird ionosphere header ---------------------------------------------*/
static int decode_ion_head(rtcm_t *rtcm, int *sync, double *udint, int *minLat, int *maxLat,
                           double *intervalOfLat, int *minLon, int *maxLon,
                           double *intervalOfLon, int *height, int *iodi)
{
    double tow;
    char *msg;
    int i=24+12, udi, lai, loi;
    ...
    *minLat        =getbitu(rtcm->buff,i, 10); i+= 10;
    *maxLat        =getbitu(rtcm->buff,i, 10); i+= 10;
    lai            =getbitu(rtcm->buff,i,  2); i+=  2;
    *intervalOfLat =intervalOfIon[lai];


    *minLon        =getbitu(rtcm->buff,i, 10); i+= 10;
    *maxLon        =getbitu(rtcm->buff,i, 10); i+= 10;
    loi            =getbitu(rtcm->buff,i,  2); i+=  2;
    *intervalOfLon =intervalOfIon[loi];

    *height =getbitu(rtcm->buff,i, 10); i+= 10;
    *iodi   =getbitu(rtcm->buff,i, 2 ); i+=  2;


    trace(4,"decode_ion_head: time=%s sync=%d udint=%.0f minLat=%d maxLat=%d minLon=%d maxLon=%d\n",
          time_str(rtcm->time,2),*sync, *udint, *minLat, *maxLat, *minLon, *maxLon);

    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
        sprintf(msg," %s udint=%.0f minLat=%d maxLat=%d minLon=%d maxLon=%d",
                time_str(rtcm->time,2),*udint, *minLat, *maxLat, *minLon, *maxLon);
    }

    return 1;
}
/* decode grid ionosphere corrections -------------------------------------------*/
static int decode_ion(rtcm_t *rtcm)
{
    double udint,intervalOfLat,intervalOfLon;
    int i,j,k,n,type,sync,minLat,maxLat,minLon,maxLon,height,iodi;

    type=getbitu(rtcm->buff,24,12);

    if (!(decode_ion_head(rtcm,&sync,&udint,&minLat,&maxLat,&intervalOfLat,&minLon,&maxLon,&intervalOfLon,&height,&iodi))) {
        trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    rtcm->ion.t             = rtcm->time;
    rtcm->ion.udi           = udint;
    rtcm->ion.minLat        = minLat;
    rtcm->ion.maxLat        = maxLat;
    rtcm->ion.intervalOfLat = intervalOfLat;
    rtcm->ion.minLon        = minLon;
    rtcm->ion.maxLon        = maxLon;
    rtcm->ion.intervalOfLon = intervalOfLon;
    rtcm->ion.height        = height;
    rtcm->ion.iodi          = iodi;

    j = (maxLat-minLat)/intervalOfLat;
    k = (maxLon-minLon)/intervalOfLon;
    rtcm->ion.ionData = (ionData_t *)malloc(j*k*sizeof(ionData_t));

    n=(maxLon-minLon)/intervalOfLon+1;
    for(j=minLat;j<=maxLat;j+=intervalOfLat)
        for (k=minLon;k<=maxLon;k+=intervalOfLon)
        {
            rtcm->ion.ionData[(j-minLat)*n+(k-minLon)].data  = getbits(rtcm->buff,i,9); i+=9;
            rtcm->ion.ionData[(j-minLat)*n+(k-minLon)].givei = getbits(rtcm->buff,i,4); i+=4;
        }
    rtcm->ion.update  = 1;
    return sync?0:10;
}
...
static int decode_msm4(rtcm_t *rtcm, int sys)
{
    ...
    (rtcm,sys,&h,r,pr,cp,NULL,NULL,cnr,lock,NULL,half);
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    if (type == 28)
        type =4011;
    if( type != 1004 && type!= 1005 && type != 1124 && type != 1012 && 4011 !=type && 1019 != type && 1020 != type )
    {
        return 0;
    }
    switch (type) {
        ...
        case 4011: ret=decode_type4011(rtcm);     break;
        case 4013: ret=decode_ion(rtcm);break;
        case 4028: ret=decode_ssr4(rtcm,SYS_CMP); break;
        case 4027: ret=decode_ssr2(rtcm,SYS_CMP); break;
        case 4026: ret=decode_ssr1(rtcm,SYS_CMP); break;
