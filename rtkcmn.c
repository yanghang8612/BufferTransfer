...
const double chisqr[100]={      /* chi-sqr(n) (alpha=0.001) ,?????ֲ?*/
...
extern int satno(int sys, int prn)
{
    ...
    switch (sys) {
       ...
       // case SYS_BDS:
        ...
        //    return NSATGPS + NSATGLO+ NSATGAL + NSATQZS + NSATCMP + NSATSBS + prn - MINPRNBDS + 1;
    }
    return 0;
}
...
/* compare beidou ephemeris ---------------------------------------------------------*/
static int cmpbeph(const void *p1, const void *p2)
{
    beph_t *q1=(beph_t *)p1,*q2=(beph_t *)p2;
    return q1->ttr.time!=q2->ttr.time?(int)(q1->ttr.time-q2->ttr.time):
           (q1->toe.time!=q2->toe.time?(int)(q1->toe.time-q2->toe.time):
            q1->sat-q2->sat);
}
/* sort and unique beidou ephemeris -------------------------------------------------*/
static void uniqbeph(nav_t *nav)
{
    beph_t *nav_eph;
    int i,j;
    ...
    qsort(nav->beph,nav->n,sizeof(beph_t),cmpbeph);

    for (i=j=0;i<nav->n;i++) {
        if (nav->beph[i].sat!=nav->beph[j].sat||
            nav->beph[i].iode!=nav->beph[j].iode) {
            nav->beph[++j]=nav->beph[i];
        }
    }
    nav->n=j+1;

    if (!(nav_eph=(beph_t *)realloc(nav->eph,sizeof(beph_t)*nav->n))) {
        //trace(1,"uniqeph malloc error n=%d\n",nav->n);
        free(nav->beph); nav->beph=NULL; nav->n=nav->nmax=0;
        return;
    }
    nav->beph=nav_eph;
    nav->nmax=nav->n;
...
/*extern void* FindMemoryInfoFunc(int memoryid, int size)
{
    if(memoryid !=0 && size != 0)
        return NULL;
    return NULL;
}*/
#endif /* EXTLEX */
