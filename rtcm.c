...
*-----------------------------------------------------------------------------*/
#include "rtklib.h"
//#include "servelibraryfunction.h"
//#include "QDebug"
//#include "QString"
...
{
   // char *gggg = (char *)FindMemoryInfoFunc(801, sizeof(eph_t)*32+4);
    gtime_t time0={0};
    obsd_t data0={{0}};
    eph_t  eph0 ={0,-1,-1};
    beph_t  beph0 ={0,-1,-1};
    ...
    if (!(rtcm->obs.data=(obsd_t *)malloc(sizeof(obsd_t)*MAXOBS))||
        ...
        !(rtcm->nav.geph=(geph_t *)malloc(sizeof(geph_t)*MAXPRNGLO))||
         !(rtcm->nav.beph=(beph_t *)malloc(sizeof(beph_t)*MAXPRNCMP))) {
        free_rtcm(rtcm);
        return 0;
    }
    rtcm->obs.n=0;
    ...
    for (i=0;i<MAXPRNCMP;i++) rtcm->nav.beph[i]=beph0;
    return 1;
}
...
extern int init_nav(nav_t *nav)
{
    ...
    nav->beph=NULL;
    /* reallocate memory for observation and ephemris buffer */
    if (!(nav->eph =(eph_t  *)malloc(sizeof(eph_t )))||
        !(nav->geph=(geph_t *)malloc(sizeof(geph_t)))||
          !(nav->beph=(beph_t *)malloc(sizeof(geph_t)))  ) {
        free_nav(nav);
        return 0;
    }
   // rtcm->obs.n=0;
...
extern void free_rtcm(rtcm_t *rtcm)
{
    ...
    free(rtcm->nav.beph); rtcm->nav.beph=NULL; rtcm->nav.nc=0;
}
...
extern int input_rtcm3(rtcm_t *rtcm, unsigned char data)
{
  ...
  //  int type=/869(rtcm->buff,24,12);

    /* check parity */
    int temp =getbitu(rtcm->buff, rtcm->len*8,24);
    int key =crc24q(rtcm->buff, rtcm->len);
   // if (crc24q(rtcm->buff,rtcm->len)!=getbitu(rtcm->buff,rtcm->len*8,24)) {
    ...
    trace(2,"rtcm3 parity error: len=%d\n",rtcm->len);
   // qDebug()<<"decode datetype"+QString::number(type);
...
* return : status (1:ok,0:error)
*-----------------------------------------------------------------------------*/
extern int gen_rtcm3(rtcm_t *rtcm, unsigned char *data, int type, int sync)
{
    unsigned int crc;
    int i=0;
    ...
    memset(data, rtcm->len, rtcm->nbyte);
    //data =rtcm->nbyte;
    return 1;
}

extern int gen_type4030(Type4030 *data4030)
{
    unsigned int crc;
    int i=0;
   ...
   // rtcm->nbit=rtcm->len=rtcm->nbyte=0;
    ...
    /* set preamble and reserved */
    setbitu(data4030->buff,i, 8,RTCM3PREAMB); i+= 8;
  //  setbits(data4030->buff, i, 32, 4096);
    setbitu(data4030->buff,i, 6,0          ); i+= 6;
    setbitu(data4030->buff,i,10,0          ); i+=10;

    if (!encode_type4030(data4030)) return 0;
    setbitu(data4030->buff,14,10,13);

    /*for (i=0;i%8;i++) {
        setbitu(data4030->buff,i,1,0);
    }*/
    crc=crc24q(data4030->buff,16);
    setbitu(data4030->buff,128,24,crc);
    //memset(data, rtcm->len, rtcm->nbyte);
    return 1;
}
